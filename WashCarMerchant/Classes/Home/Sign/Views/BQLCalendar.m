...
@interface BQLCalendar ()
{
    ...
    UILabel *centerLab;//显示月份日期lab
}

@property (nonatomic, strong) UIView *baseView;//放置时间
@property (nonatomic, assign) int dayIndex ;//加减月份
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        CGFloat top = 30 +(self.frame.size.height -30)/6.0;
        
        _baseView.frame = CGRectMake(0, top, self.frame.size.width, self.frame.size.height-top);
       _baseView.backgroundColor = WhiteColor;
        [self addSubview:_baseView];
        
        UISwipeGestureRecognizer *leftSwipe = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(panGestureRecognizer:)];
        leftSwipe.direction = UISwipeGestureRecognizerDirectionLeft;
        [_baseView addGestureRecognizer:leftSwipe];
        UISwipeGestureRecognizer *rightSwipe = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(panGestureRecognizer:)];
        rightSwipe.direction = UISwipeGestureRecognizerDirectionRight;
        [_baseView addGestureRecognizer:rightSwipe];
        
        daysWidth  = _baseView.frame.size.width / 7.f;
        daysHeight = _baseView.frame.size.height / 6.f;
...
- (void)panGestureRecognizer:(UISwipeGestureRecognizer *)pan{
...
- (void)initSign:(NSArray *)signArray Touch:(calendarBlock )block {
    ...
    self.clickDayBlock = block;
    _signedArray = signArray;
    
    NSDateFormatter *dateFormatter =  [[NSDateFormatter alloc]init];
    dateFormatter.dateFormat = @"YYYY-MM";
    NSString *dateString =  [dateFormatter stringFromDate:[self getSelectCalendar:YES]];
    //   centerLab.text =  [LLUtils dateStrWithDate:date dateFormat:@"YYYY-MM"];
    
    centerLab.text = dateString;
    
    [self createCalendarViewWith:[self getSelectCalendar:NO]];
}
- (NSDate *)getSelectCalendar:(BOOL)isSetDayToOne
{
    ...
    [components setMonth:([components month] - _dayIndex)];
    NSDate * currentMonthDate = [cal dateFromComponents:components];
...
- (void)clickLeftBtn{
    ...
    if (self.changeMonthBlock) {
        self.changeMonthBlock([self getSelectCalendar:YES]);
    }
}
...
- (void)setTopUI//设置顶端界面
{
    UIButton *leftBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    leftBtn.frame = CGRectMake(10, 5, 30, 40);
    ...
    rightBtn.frame = CGRectMake(self.frame.size.width-40,  5, 30, 40);
    [rightBtn setImage:[UIImage imageNamed:@"back"] forState:0];
    rightBtn.imageView.transform = CGAffineTransformMakeRotation(M_PI);
    [rightBtn addTarget:self  action:@selector(clickRightBtn) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:rightBtn];
    
    centerLab = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 80, 30)];
    centerLab.center = self.center;
    CGPoint center = centerLab.center;
    center.y = rightBtn.center.y;
    centerLab.center = center;
    ...
    weekdayBackgroundView.frame = CGRectMake(0, 40, selfWidth, daysHeight);
...
- (void)createCalendarViewWith:(NSDate *)date {
    ...
    [self removeAllSubviews:self.baseView];
    ...
    NSInteger row = (firstWeekday+daysInThisMonth)/7 + ((firstWeekday+daysInThisMonth)%7 == 0 ? 0 : 1);
    
    NSInteger count = row*7;
    
    __weak BQLCalendar *selfWeak = self;
    
    CGRect selfFrame = self.frame;
    selfFrame.size.height = selfHeight - (row == 5 ? daysHeight : 0);
    
    [UIView animateWithDuration:0.4 animations:^{
        selfWeak.frame = selfFrame;
    }];
...
#warning the date include last month、this month、next month
        ...
        UILabel *chineseCalendar = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(dayButton.frame), CGRectGetWidth(view.frame), 17)];
        
        chineseCalendar.textAlignment = 1;
//        chineseCalendar.textColor = RGB(210, 210, 210);
        
        chineseCalendar.textColor = RGBAlpha(21, 170, 155, 1.0);
...
//        chineseCalendar.text = nowDate;
        ...
        // this month
        if ([self month:date] == [self month:date]) {
...
//            if (i <= todayIndex && i >= firstWeekday) {
                ...
                if([_signedArray containsObject:[NSString stringWithFormat:@"%d",[str intValue]]]){
                    [self setStyle_BeforeToday:dayButton andIsSign:YES];
                    chineseCalendar.text = @"已签到";
...
- (void)dateTouch:(UIButton *)sender {
    ...
    if(self.clickDayBlock) {
        
        self.clickDayBlock([dateFormatter dateFromString:date]);
    }
}
