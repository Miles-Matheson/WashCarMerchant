...
//多张图片
- (void)createUIWithPicInfoArr:(NSMutableArray *)marr
{
    //获取当前显示的窗口
    _window = [UIApplication sharedApplication].keyWindow;
...
//    currentPage = index;
    ...
    //把当前显示的图片位置设为原始frame 再通过动画展示出来
    [self createAnimationShowPicWithIndex:0];
...
//        [subScr addSubview:containerView];
        ...
        [picImageView sd_setImageWithURL:[NSURL URLWithString:info.picUrlStr] placeholderImage:[UIImage imageNamed:PlaceHolderImageName] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    if (error) {
        ...
        NSLog(@"成功");
//        WSMakeToastInKeyWindow(@"保存成功!");
//        [KeyWindow makeToast:@"保存成功!" duration:0.5 position:CSToastPositionCenter];
    }
}
